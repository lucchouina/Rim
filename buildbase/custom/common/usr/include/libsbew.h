/*
 * $Id: libsbew.h,v 2.7 2008/04/07 18:25:24 rdobbs C1T3_REL_2_0B $
 */
 ...
 *-----------------------------------------------------------------------------
 * RCS info:
 * RCS revision: $Revision: 2.7 $
 * Last changed on $Date: 2008/04/07 18:25:24 $
 * Changed by $Author: rdobbs $
 *-----------------------------------------------------------------------------
 * $Log: libsbew.h,v $
 * Revision 2.7  2008/04/07 18:25:24  rdobbs
 * Add new recvclk[12] card parameters to allow selection of Tect3 Master Clock Source.
 *
 * Revision 2.6  2007/04/09 17:56:08  rickd
 * Fix typo for CH_PROTO_ISPL -> ISLP to match 256T3, where-its-used.
 *
 * Revision 2.5  2007/04/02 17:13:29  rickd
 * New company - One Stop Systems.
 * Rename channel parameter mode -> chan_mode.
 *
 * Revision 2.4  2007/03/19 18:55:07  rickd
 * Replace reserved area w/in config struct with [rt]xdesc_XXX variables.
 * Add new rx_onr_errors & tx_busy_errors to statistics.
 *
 * Revision 2.3  2006/01/18 19:43:10  rickd
 * Add CFG_CLK option settings.
 *
 * Revision 2.2  2005/06/06 20:40:57  rickd
 * Add CFG_FRAME_*_AMI values for C4T1E1 generic usage support.
 *
 * Revision 2.1  2005/01/11 18:52:10  rickd
 * Add brd_addr structure for BRDADDR_GET Ioctl support.  NCOMM needs our
 * hardware address.
 *
 * Revision 2.0  2004/11/29 21:43:50  rickd
 * Add hardware id into brd_info.  Framer support get/set ioctls
 * added to common library: name change and reserved[] addition for
 * future support.
 *
 *-----------------------------------------------------------------------------
 */
...
/* routine/ioctl: wancfg_set_chan_param() - SBE_IOC_CHAN_SET */
    ...
    struct sbecom_chan_param
    {
        ...
        /* the following are for reporting purposes and are currently read-only */
        u_int16_t  txdesc_current; /* E: minimum TxDesc used */
        u_int16_t  rxdesc_inc;     /* 10: increment RxDesc count upon ONR recovery */
        u_int16_t  rxdesc_min;     /* 12: minimum RxDesc used */
        u_int16_t  rxdesc_max;     /* 14: maximum RxDesc can grow to */
        u_int16_t  rxdesc_current; /* 16: minimum RxDesc used */
    };
...
/* routine/ioctl: wancfg_get_card_chan_stats() - SBE_IOC_CARD_CHAN_STAT */
    ...
    struct sbecom_chan_stats
    {
        ...
        /* MSCC/MUSYCC specific errors: */
        unsigned long rx_onr_errors;    /* buffer ownership, channel
                                         * disabled */
        unsigned long tx_busy_errors;   /* start_xmit() received pkt after sd_disable_xmit() */
...
/****************************************/
    ...
    struct sbecom_card_param
    {
        ...
        u_int8_t    recvclk1;    /* F: Refclk1 framer selection */
        u_int8_t    recvclk2;    /* 0: Refclk2 framer selection */
        u_int8_t    reserved[1 + 14];   /* reserved for expansion - must be
                                         * ZERO filled */
    };
...
/* routine/ioctl: wancfg_set_framer() - SBE_IOC_FRAMER_SET */
    ...
    {
        u_int8_t    framer_num; /* T1: 1-28, E1: 1-21 */
...
/* framer numbers <framer_num>
 *
 * IMPLEMENTATION NOTE: Per TEMUX register descriptions "when the TEMUX
 * DS3 mux is configured in G.747 mode every fourth framer must be held
 * in reset (4,8,12,etc)".  However, the user usually is working in the
 * non-hardware environment wherein T1 Framers are numbered 1-28, and E1
 * Framers are numbered 1-21.  The user interface will present the
 * contiguous base ONE numbering scheme, and the driver will implement transparent
 * remapping of framer indexes in order to access the correct hardware
 * framers.
 */
...
#define CFG_FRAME_E1CRC_AMI        9    /* AMI */
#define CFG_FRAME_E1CRC_CAS_AMI   10    /* AMI */
...
/**************************************************************/
    ...
    extern int  wancfg_get_recvclk (wcfg_t *, struct sbecom_wrt_vec *);
    ...
    extern int  wancfg_set_recvclk (wcfg_t *, struct sbecom_wrt_vec *);
