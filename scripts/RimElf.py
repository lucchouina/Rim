...
class ELFSection:
    ...
    def __init__(self, is64=0, entSize=0):
        ...
        self.lma =  None
        if is64:
            self.Elf_Shdr = "<IIQQQQIIQQ"
        else:
            self.Elf_Shdr = "<IIIIIIIIII"
        self.entSize=entSize
        self.hdrSize=struct.calcsize(self.Elf_Shdr)

    def fromFile(self, fileobj):
        """get section header from string"""
        shdr = (fileobj.read(self.entSize) + '\0'* self.hdrSize)[0:self.hdrSize]
        (self.sh_name, self.sh_type, self.sh_flags, self.sh_addr,
         self.sh_offset, self.sh_size, self.sh_link, self.sh_info,
         self.sh_addralign, self.sh_entsize) = struct.unpack(self.Elf_Shdr, shdr)
...
class ELFProgramHeader:
    ...
    def __init__(self, is64=0, entSize=0):
        ...
        self.data = None
        if is64:
            self.Elf_Phdr = "<IIQQQQQQ"
        else:
            self.Elf_Phdr = "<IIIIIIII"
        self.entSize=entSize
        self.hdrSize=struct.calcsize(self.Elf_Phdr)
    ...
    def fromFile(self, fileobj):
        ...
        (self.p_type, self.p_offset, self.p_vaddr, self.p_paddr,
            ...
            self.p_align) = struct.unpack(self.Elf_Phdr, shdr)
...
class ELFObject:
    ...
    #header information
    Elf32_Ehdr = "HHIIIIIHHHHHH"
    Elf64_Ehdr = "HHIQQQIHHHHHH"
    ...
    def fromFile(self, fileobj):
        ...
        # Get basic info - Byte Order / Bits (32/64)
        unpackString = "BBBBB"
        unpackSize = struct.calcsize(unpackString)
        Class, Encoding, Version, OsAbi, AbiVersion = struct.unpack(unpackString, buf[4 : 4+unpackSize])
        # check bit size
        if Class == self.ELFCLASS32:
            hdrFmt=self.Elf32_Ehdr
            self.is64=0
        elif Class == self.ELFCLASS64:
            hdrFmt=self.Elf64_Ehdr
            self.is64=1
        else:
            raise ELFException("Unsupported Class")
        # check by order
        if Encoding != self.ELFDATA2LSB:
            raise ELFException("Unsupported byte order")
        sHdrSize=struct.calcsize(hdrFmt)
        buf=fileobj.read(sHdrSize)
        (self.e_type, self.e_machine, self.e_version,
        self.e_entry, self.e_phoff, self.e_shoff,
        self.e_flags, self.e_ehsize, self.e_phentsize, self.e_phnum,
        self.e_shentsize, self.e_shnum, self.e_shstrndx) = struct.unpack( hdrFmt, buf)
        ...
        if self.e_phnum:
            ...
            for sectionnum in range(self.e_phnum):
                psection = ELFProgramHeader(self.is64, self.e_phentsize)
                psection.fromFile(fileobj)
        ...
        for sectionnum in range(self.e_shnum):
            elfsection = ELFSection(self.is64, self.e_shentsize)
            elfsection.fromFile(fileobj)
            self.sections.append(elfsection)
        
        #load data for all sections
        for section in self.sections:
            fileobj.seek(section.sh_offset)
            data = fileobj.read(int(section.sh_size))
            section.data = data
...
if __name__ == '__main__':
    ...
    #    raise Exception("No executable type is %d" % elf.e_type)
    print elf
